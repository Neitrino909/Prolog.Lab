...
mother(X,Y):-woman(X),parent(X,Y),write("явл€етс€"),nl ,fail.
mother(X):-woman(Y),parent(Y,X),write(Y),nl ,fail.

father(X,Y):-man(X),parent(X,Y),write("явл€етс€"),nl ,fail.
father(X):-man(Y),parent(Y,X),write(Y),nl ,fail.

brother(X,Y):-man(X),parent(Z,X),parent(Z,Y),write("явл€етс€"),!,nl,fail.
brothers(X):-parent(Z,X),parent(Z,Y),man(Y),man(Z),dif(X,Y),write(Y),nl,fail.

wife(X,Y):-woman(X),parent(X,Z),parent(Y,Z),write("явл€етс€"),!,nl,fail.
wife(X):-woman(Y),parent(X,Z),parent(Y,Z),write(Y),!,nl,fail.

b_s(X,Y):-parent(Z,X),parent(Z,Y),write("явл€ютс€"),!,nl,fail.
b_s(X):-parent(Z,X),parent(Z,Y),woman(Z),not(X=Y),write(Y),nl,fail.

grand_pa(X,Y):-man(X),parent(X,Z),parent(Z,Y),write("явл€етс€"),nl,fail.
grand_pas(X):-man(Y),parent(Z,X),parent(Y,Z),write(Y),nl,fail.

grand_ma(X,Y):-woman(X),parent(X,Z),parent(Z,Y),write("явл€етс€"),nl,fail.
grand_mas(X):-woman(Y),parent(Z,X),parent(Y,Z),write(Y),nl,fail.

grand_pa_and_son(X,Y):-(parent(X,Z),parent(Z,Y),man(X))\=(parent(Z,X),parent(Z,Y)),man(X),man(Y),write("явл€ютс€"),nl,fail.
